...
/// @notice Interface for the on-chain PNG icon registry
...
interface IIconRegistry {
    ...
    /// @notice Get icon as base64 PNG data URI for direct use in <img src="">
    ...
    /// @notice Get token icon as PNG data URI
    /// @dev Gas-heavy; intended for off-chain use
    /// @param token Token contract address
    /// @param chainId Chain ID where token is deployed
    ...
    /// @return Array of PNG icon bytes (empty for missing icons)
    ...
    /// @return Array of PNG icon bytes (empty for unmapped tokens)
